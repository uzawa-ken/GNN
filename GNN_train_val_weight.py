...
def train_gnn_auto_trainval_pde_weighted(
    ...
    *,
    ...
    log_print(f"[INFO] (time, rank) ペアの数: {n_total}")
    log_print(f"[INFO] train: {n_train} cases, val: {n_val} cases")
    ...
    log_print(f"[INFO] nFeatures: {nFeat}, 総セル数: {total_cells}")
    ...
    if LAMBDA_DATA == 0:
        learning_mode = "完全な教師なし学習 (PDE 損失のみ)"
    elif LAMBDA_PDE == 0:
    ...
    log_print(f"    DATA 損失の重み: {LAMBDA_DATA}, PDE 損失の重み: {LAMBDA_PDE}")
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if epoch % PLOT_INTERVAL == 0 or epoch == 1:
            ...
            log = (
                f"[Epoch: {epoch:5d}] 損失: {loss_value:.4e}, "
                f"学習率: {current_lr:.3e}, "
